...
export default function ConversationsLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-full w-full bg-white dark:bg-slate-950">
      {/* Master List Column */}
      <div 
        className={`${isChatView ? 'hidden md:flex' : 'flex'} w-full md:w-[380px] lg:w-[420px] h-full flex-col border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 z-10 transition-all shadow-sm`}
      >
        <ConversationList />
      </div>
      ...
      {/* Detail Chat Column */}
      <div 
        className={`${!isChatView ? 'hidden md:flex' : 'flex'} flex-1 h-full flex-col bg-[#F0F2F5] dark:bg-[#0B141A] relative z-0`}
      >
