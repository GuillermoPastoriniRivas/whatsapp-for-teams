...
export function agentInvitationEmail(params: AgentInvitationParams): { subject: string; html: string; text: string } {
  ...
  const subject = `Te invitaron a unirte a ${tenantName} en Fluws`;

  const html = wrapInLayout(
    `<p style="margin:0 0 16px;">Hola <strong>${agentName}</strong>,</p>
<p style="margin:0 0 16px;"><strong>${inviterName}</strong> te invitó a unirte al equipo de <strong>${tenantName}</strong> en Fluws.</p>
...
<p style="margin:0;font-size:13px;color:${GRAY_500};">Si no esperabas esta invitación, podés ignorar este email.</p>`,
    `${inviterName} te invitó a unirte a ${tenantName} en Fluws`,
  );

  const text = `Hola ${agentName},

${inviterName} te invitó a unirte al equipo de ${tenantName} en Fluws.
