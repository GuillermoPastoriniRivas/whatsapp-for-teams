...
export default function ConversationsLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-full w-full bg-background">
      {/* Master List Column */}
      <div 
        className={`${isChatView ? 'hidden md:flex' : 'flex'} w-full md:w-[380px] lg:w-[420px] h-full flex-col border-r border-border bg-background z-(--z-sticky) transition-all shadow-sm`}
      >
        <ConversationList />
      </div>
      ...
      {/* Detail Chat Column */}
      <div 
        className={`${!isChatView ? 'hidden md:flex' : 'flex'} flex-1 min-w-0 h-full flex-col bg-[var(--asis-surface-panel)] relative z-0 overflow-hidden`}
      >
        ...
        <div className="relative z-(--z-sticky) h-full flex flex-col">
          {children}
        </div>
      </div>
    </div>
