...
const STEM = 'M116 256H272';
const UPPER = 'M272 256C304 256 336 212 396 152';
const LOWER = 'M272 256C304 256 336 300 396 360';
const STROKE = 62;
...
const strokes = (color) =>
  ...
  `<path d="${STEM}"/><path d="${UPPER}"/><path d="${LOWER}"/></g>`;
...
/**
 * La horquilla blanca sobre el cuadrado teal.
...
const appSvg = ({ bleed = false, glyph = bleed ? 0.58 : 0.72 } = {}) =>
  ...
  <g transform="translate(256 256) scale(${glyph}) translate(-256 -256)">${strokes('white')}</g>
</svg>`;
...
(async () => {
  ...
  const [
    ...
    render(badgeSvg(), 72),
    // El favicon lleva el glifo más grande: a 16px la horquilla al 72% se
    // empasta y queda un cuadrado teal con una mancha blanca en el medio.
