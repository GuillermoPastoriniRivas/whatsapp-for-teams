...
export interface PhoneNumberRepository {
  create(
    phoneNumber: Omit<PhoneNumber, 'id' | 'createdAt' | 'portfolioId' | 'bsuidScope' | 'businessProfile'> & {
  ...
  update(id: string, data: Partial<Pick<PhoneNumber, 'label' | 'status' | 'webhookSecret' | 'providerConfig' | 'wabaId' | 'phoneNumberId' | 'displayPhone' | 'portfolioId' | 'businessProfile'>>): Promise<PhoneNumber | null>;
