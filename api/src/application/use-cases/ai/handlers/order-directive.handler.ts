...
export class OrderDirectiveHandler {
  ...
  async handleAction(
    ...
    if (items.length === 0) {
      ...
      return { message: 'No items provided for order', orderId: '' };
    }
    ...
    return { message: `Order created successfully (${items.length} items, total: ${params.total ?? 'N/A'})`, orderId: result.value.id };
  }
}
